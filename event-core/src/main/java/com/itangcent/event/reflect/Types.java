...
final class Types {
    ...
    static Type newArrayType(Type componentType) {
        if (componentType instanceof WildcardType) {
            ...
            Assert.isTrue(lowerBounds.length <= 1, "Wildcard cannot have more than one lower bounds.");
            if (lowerBounds.length == 1) {
                return supertypeOf(newArrayType(lowerBounds[0]));
            } else {
                Type[] upperBounds = wildcard.getUpperBounds();
                Assert.isTrue(upperBounds.length == 1, "Wildcard should have only one upper bound.");
    ...
    static ParameterizedType newParameterizedTypeWithOwner(
        ...
        Assert.isTrue(rawType.getEnclosingClass() != null, "Owner type for unenclosed %s", rawType);
    ...
    private static final class ParameterizedTypeImpl implements ParameterizedType, Serializable {
        ...
        ParameterizedTypeImpl(Type ownerType, Class<?> rawType, Type[] typeArguments) {
            ...
            Assert.isTrue(typeArguments.length == rawType.getTypeParameters().length);
        ...
        public String toString() {
            ...
            return builder
                    ...
                    .append('<')
                    .append(StringUtils.join(argumentsList.toArray(), ','))
                    .append('>')
                    .toString();
        }
    ...
    private static final class TypeVariableInvocationHandler implements InvocationHandler {
        ...
        static {
            Map<String, Method> map = Collections.newHashMap();
    ...
    private static void disallowPrimitiveType(Type[] types, String usedAs) {
        for (Type type : types) {
            if (type instanceof Class) {
                ...
                Assert.isTrue(!cls.isPrimitive(), "Primitive type '%s' used as %s", cls, usedAs);
            }
        }
    }
    ...
    enum JavaVersion {
        ...
        final List<Type> usedInGenericType(Type[] types) {
            List<Type> list = Collections.newArrayList();
            for (Type type : types) {

...
import com.itangcent.event.utils.Primitives;
...
public abstract class TypeToken<T> extends TypeCapture<T> implements Serializable {
    ...
    protected TypeToken() {
        ...
        Assert.notInstanceOf(
                TypeVariable.class, runtimeType,
    ...
    private TypeToken(Type type) {
        this.runtimeType = Objects.requireNonNull(type);
    }
    ...
    final List<TypeToken<? super T>> getGenericInterfaces() {
        ...
        List<TypeToken<? super T>> tokens = Collections.newArrayList();
    ...
    public final Invokable<T, T> constructor(Constructor<?> constructor) {
        ...
        return new Invokable.ConstructorInvokable<T>(constructor) {
            ...
            @Override
            public String toString() {
                return getOwnerType() + "(" + StringUtils.join(getGenericParameterTypes(), ',') + ")";
            }
        };
    }
    ...
    public class TypeSet implements Serializable {
        ...
        public Set<TypeToken<? super T>> allTypes() {
            ...
            if (filteredTypes == null) {
                ...
                List<TypeToken<? super T>> collectedTypes =
                        (List) TypeCollector.FOR_GENERIC_TYPE.collectTypes(TypeToken.this);
                return (types =
        ...
        public Set<Class<? super T>> rawTypes() {
            ...
            return Collections.newHashSet(collectedTypes);
        }
    ...
    private final class ClassSet extends TypeSet {
        ...
        public Set<TypeToken<? super T>> allTypes() {
            ...
            if (result == null) {
                ...
                return (classes = collectedTypes.stream()
                        .filter(TypeFilter.IGNORE_TYPE_VARIABLE_OR_WILDCARD)
                        .collect(Collectors.toSet()));
            } else {
                return result;
        ...
        public Set<Class<? super T>> rawTypes() {
            ...
            @SuppressWarnings({"unchecked", "rawtypes"})
            List<Class<? super T>> collectedTypes =
                    (List) TypeCollector.FOR_RAW_TYPE.classesOnly().collectTypes(getRawTypes());
            return Collections.newHashSet(collectedTypes);
        }
    ...
    private Set<Class<? super T>> getRawTypes() {
        final Set<Class<?>> set = Collections.newHashSet();
        new TypeVisitor() {
            @Override
            ...
            void visitGenericArrayType(GenericArrayType t) {
                set.add(Types.getArrayClass(of(t.getGenericComponentType()).getRawType()));
            }
        }.visit(runtimeType);
        // Cast from Set<Class<?>> to Set<Class<? super T>>
        @SuppressWarnings({"unchecked", "rawtypes"})
        Set<Class<? super T>> result = (Set) set;
        return result;
    }

    private boolean isOwnedBySubtypeOf(Type supertype) {
        for (TypeToken<?> type : getTypes().allTypes()) {
    ...
    private TypeToken<? super T> getArraySupertype(Class<? super T> supertype) {
        ...
        Assert.notNull(componentType, "%s isn't a super type of %s", supertype, this);
        ...
        final TypeCollector<K> classesOnly() {
            return new ForwardingTypeCollector<K>(this) {
                ...
                List<K> collectTypes(Iterable<? extends K> types) {
                    List<K> typeList = Collections.newArrayList();
                    for (K type : types) {
                        if (!getRawType(type).isInterface()) {
                            typeList.add(type);
                        }
                    }
                    return super.collectTypes(typeList);
                }
            };
        }

        final List<K> collectTypes(K type) {
            return collectTypes(java.util.Collections.singletonList(type));
        }

        List<K> collectTypes(Iterable<? extends K> types) {
            // type -> order number. 1 for Object, 2 for anything directly below, so on so forth.
            Map<K, Integer> map = Collections.newHashMap();
            for (K type : types) {
